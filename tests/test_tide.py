...
class TideHelpersTest(unittest.TestCase):
    """Tests for selecting and formatting tide data."""

    def test_selects_first_future_tide_without_using_external_ids(self):
        """The external @id does not need to match the list position."""
        self.assertEqual(
            get_next_tide(TODAY, TOMORROW, datetime(2026, 8, 8, 7, 0)),
            TODAY[1],
        )

    def test_exact_tide_minute_selects_the_following_tide(self):
        """Preserve the legacy boundary behaviour at the tide minute."""
        self.assertEqual(
            get_next_tide(TODAY, TOMORROW, datetime(2026, 8, 8, 7, 15)),
            TODAY[2],
        )

    def test_after_last_tide_selects_tomorrow(self):
        ...
        self.assertEqual(tide, TOMORROW)
        self.assertTrue(is_tomorrow)

    def test_skips_malformed_hours(self):
        """Malformed entries do not prevent selecting a valid tide."""
        tides = [{"@hora": "invalid"}, TODAY[2]]
        self.assertEqual(
            get_next_tide(tides, TOMORROW, datetime(2026, 8, 8, 10, 0)),
            TODAY[2],
        )

    def test_legacy_state_text_is_unchanged(self):
        """Existing automation-facing state values remain unchanged."""
        self.assertEqual(get_state_from_tide(TODAY[0]), "High tide at 01:00")
        self.assertEqual(get_state_from_tide(TODAY[1]), "Low tide at 07:15")
